...
params <- list(
  ...
  real_withdrawal = 40000, # Fixed real income target, retirment withdrawal
...
# 3a. Simulation death age based on life table
...
# 3c. Simulate single account path
simulate_path <- function(params, lt){
  ...
  for (t in 1:(params$max_age - params$start_age)){
    ...
    # Cash flow logis: pre-retirement vs post-retirement
...
run_monte_carlo <- function(params, lt) {
  ...
  TVaR_5    <- mean(terminal_balance[terminal_balance <= VaR_5]) # The average terminnal balance in the worst 5% of scenarios, captures tail severity.
